...
public class AI3 extends AI
{
	...
	protected FastArrayList<Move> availableMoves = null; // all the moves available at root
	...
	public Move selectAction(final Game game,final Context context,final double secondsPerMove,final int maxIterations,final int maxDepth)
	{
		...
		if (movesTillNow<=70 || remainingGameTime>=150)
		{
		moveTimeInSeconds=remainingGameTime/(85-movesTillNow);
		}
		else
		{
			moveTimeInSeconds=remainingGameTime/(100-movesTillNow);
		}
...
//				if (AspirationWindow)
				//{ALPHA = -windowValue;BETA = +windowValue;}
				//else {ALPHA = -1000000;BETA = 1000000;}
				...
				for (int i = 0; i<totalInitialMoves;i++)
				{
					...
					final int ABscore = minimaxAlphaBeta(tempContext, searchDepth-1, ALPHA, BETA, maximisingPlayer,stopTime);
